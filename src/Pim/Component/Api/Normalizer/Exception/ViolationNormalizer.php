...
class ViolationNormalizer implements NormalizerInterface
{
    ...
    protected function normalizeViolations(ConstraintViolationListInterface $violations)
    {
        ...
        foreach ($violations as $violation) {
            ...
            if ($violation->getRoot() instanceof ProductInterface &&
                preg_match('|^values\[(?P<attribute>[a-z0-9-_]+)|i', $violation->getPropertyPath(), $matches)) {
                $error = $this->productValuesErrors($violation, $matches['attribute']);
            }
     ...
     * @return array
     */
    protected function productValuesErrors(ConstraintViolationInterface $violation, $attributeCode)
    {
